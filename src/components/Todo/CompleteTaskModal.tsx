      ...
      >
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800">完成任务</h3>
              <p className="text-xs text-slate-400 mt-0.5">记录所用时间，将记入日报</p>
            </div>
          </div>
          <button 
            type="button"
        ...
        <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/70">
          <span className="text-[11px] font-semibold text-slate-400 block mb-1">任务：</span>
          ...
          {task.tags && task.tags.length > 0 && (
            <div className="flex items-center gap-1.5 mt-2">
              {task.tags.map((tg) => (
                <span key={tg} className="px-2 py-0.5 rounded-md text-[10px] bg-white border border-slate-200 text-slate-600 font-medium">
                  #{tg}
                </span>
              ))}
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="p-3 bg-slate-50/90 rounded-2xl border border-slate-200/80">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex p-0.5 bg-slate-200/70 rounded-lg text-[11px] font-semibold">
                <button
                  type="button"
            ...
            <div className="grid grid-cols-8 gap-1.5">
              {Array.from({ length: 24 }, (_, i) => i).map((hour) => {
                ...
                let btnStyle = 'bg-white hover:bg-slate-100 text-slate-700 border-slate-200/80';
                if (isStart) {
                  ...
                  btnStyle = 'bg-blue-50 text-blue-700 border-blue-200 font-semibold';
                }
                ...
                return (
                  <button
                    ...
                    className={`py-1.5 px-0.5 text-[11px] font-mono rounded-lg border text-center transition-all cursor-pointer relative ${btnStyle}`}
          ...
          <div className="grid grid-cols-2 gap-3 pt-0.5">
            <div>
              <label className="block text-[11px] font-semibold text-slate-600 mb-1 flex items-center justify-between">
                <span>开始时间</span>
                <span className={`text-[10px] ${activeTimeTarget === 'start' ? 'text-blue-600 font-bold' : 'text-slate-400'}`}>
              ...
              <input
                ...
                className={`w-full px-3 py-2 text-xs bg-slate-50 border rounded-xl text-slate-800 font-mono focus:outline-none cursor-pointer shadow-2xs transition-colors ${
                  activeTimeTarget === 'start' ? 'border-blue-500 ring-2 ring-blue-500/10' : 'border-slate-200'
                }`}
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-slate-600 mb-1 flex items-center justify-between">
                <span>结束时间</span>
                <span className={`text-[10px] ${activeTimeTarget === 'end' ? 'text-emerald-600 font-bold' : 'text-slate-400'}`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:border-blue-500 shadow-2xs"
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-slate-600 mb-1">
              ...
              <div className="flex items-center gap-1.5 px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-800 font-mono shadow-2xs">
                <Flame size={14} className="text-amber-500" />
                <span className="font-bold text-slate-900">{focusDuration}</span>
        ...
        <div className="p-3 bg-emerald-50/60 border border-emerald-100 rounded-2xl">
          ...
          <p className="text-xs text-emerald-900 font-mono leading-relaxed pl-1">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          <button
            ...
            className="text-xs font-semibold text-slate-400 hover:text-slate-600 hover:underline cursor-pointer disabled:opacity-50"
          >
            直接完成
          </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={isSubmitting}
              className="flex items-center gap-1 px-5 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 active:scale-98 rounded-xl shadow-md shadow-emerald-600/20 transition-all cursor-pointer disabled:opacity-50"
            >
